...
def saturer(im, seuil):
    imCp = im.copy() # copie de l'image
    for i in range(len(imCp)): # parcourt ligne colonne 
        for j in range(len(imCp[1])): # parcourt les couleurs
            r, v, b = im[i][j] # je leur donne chaque fois leur valeurs
